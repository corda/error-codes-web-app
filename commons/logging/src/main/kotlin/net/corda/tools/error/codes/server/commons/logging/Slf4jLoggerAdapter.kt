...
internal class Slf4jLoggerAdapter<CONTEXT : LoggingContext> private constructor(private val logger: org.slf4j.Logger) : Logger<CONTEXT> {
    ...
    override fun log(context: CONTEXT?, level: Logger.Level, message: String?, vararg args: Any?) = level.enabled()?.let(::printFormatted)?.invoke(context?.applyTo(message)
            ?: message, args) ?: Unit

    override fun log(context: CONTEXT?, level: Logger.Level, message: String?, throwable: Throwable) = level.enabled()?.let(::printWithError)?.invoke(context?.applyTo(message)
            ?: message, throwable) ?: Unit
